...
from .Auth.Auth import get_current_user, get_user_exception

# Depends stand for Dependencies
...
@app.get("/todos/user")
async def read_all_by_user(db: Session = Depends(get_db), user: dict = Depends(get_current_user)):
    if user is None:
